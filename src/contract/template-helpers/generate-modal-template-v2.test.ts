...
const { generateModalTemplateV2, ISODateToMMDDYYYY } = handlebars.helpers;
...
describe("[a97b5962] Generate Modal Template v2", () => {
  ...
  it("[ba903347] should generate a modal template when a user has another incomplete application", async () => {
    const actions = [{ action: "identify", statusCode: 201 }];
    ...
    const buttons =
      result.action.properties.definition.content[1].componentProps.buttons;
    ...
    assert.equal(
      primaryAction.properties.manifest,
      `send-access-email-on-resume/${applications[0].id}`,
    );
    assert.equal(secondaryAction.type, "request");
    assert.equal(
      secondaryAction.properties.manifest,
      `send-verification-email/${request.params.id}`,
    );
  });
  ...
  it("[ca6050ec] should generate a modal template when a user has a post submission application", async () => {
    ...
    POST_SUBMISSION_APPLICATION_STATUSES.forEach((status) => {
      ...
      const props =
        result.action.properties.definition.content[0].componentProps;
  ...
  it("[6a001d45] should generate a modal template for users with an account with no active inflight applications", async () => {
    const actions = [{ action: "identify", statusCode: 409 }];
    ...
    assert.equal(
      primaryAction.properties.goTo,
      `BASE_URL/_/auth/login?targetUrl=/_/apply/resume/${request.params.id}`,
    );
  });
  ...
  it("[06cbb7a5] should generate a modal template when a user has an account and another inflight application", async () => {
    ...
    assert.equal(
      primaryAction.properties.goTo,
      `BASE_URL/_/auth/login?targetUrl=/_/apply/resume/${applications[0].root.id}`,
    );
    assert.equal(secondaryAction.type, "navigate");
    assert.equal(
      secondaryAction.properties.goTo,
      `BASE_URL/_/auth/login?targetUrl=/_/apply/resume-new-application/${request.params.id}`,
    );
  });
});
