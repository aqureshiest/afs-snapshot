...
describe("[462fd166] manifest.execute", () => {
  ...
  it("[be0ed4d0] ajv validate keyword", async () => {
    const dobSchema = {
      type: "object",
      $id: "dobSchema",
      properties: {
        month: {
          ...
          ageOfMajority: {
            ...
            NE: 19,
            base: 18,
            statePath: "address.currentAddress.state",
            stateValue: null,
          },
          errorMessage: {
            ageOfMajority:
              "You are below the age of majority in your state of residence",
    ...
    const manifest = new Manifest(context, "manifestAJV", {
      "*": new Contract({
        key: "testContract",
        raw: `{{{ajv 'validate' 'dobSchema' (json (contract 'dob')) }}}`,
      }),
      dob: new Contract({ key: "name", raw: JSON.stringify(dob) }),
    });
    const ajv = context.loadedPlugins.schema.instance;
    ajv.compile(dobSchema);
    const { contract } = await manifest.execute(input, { context, ...input });
