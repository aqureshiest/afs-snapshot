...
export const getEndpointsFromGlob = async (): Promise<
  Array<{
    method: Glob_Method;
    endpoint: string;
    manifest: string;
...
> => {
  const globSearch = await glob("**/{GET,POST,PUT,PATCH,DELETE}.{ts,js}", {
    cwd: "dist/api/v2-proto/endpoints",
  }); // -> ["earnest/personal-loans/GET.js", ...]
