function paymentSchedules(request, state, logger) {
  ...
  return {
    ...
    body: {
      prices: prices.map((price, index) => {
        ...
        const totalInterest = Math.floor(loanInDollars * (price.rateInBps / 10000) * (price.uwLoanTermInMonths / 12));
        const priceDate = new Date(price.date)
        const lastPayDate = priceDate.setMonth(priceDate.getMonth + price.uwLoanTermInMonths)
        const minimumPaymentAmountInCents = Math.floor(price.startingPrincipalBalanceInCents / price.uwLoanTermInMonths);
